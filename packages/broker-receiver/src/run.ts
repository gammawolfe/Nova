...
// identity → builds client/dispatcher/pull-loop/health → starts, then
 ...
 *   1. Stop the pull loop (no new tasks pulled).
 *   2. Drain the dispatcher within shutdownGraceSeconds.
 *   3. Stop the health server.
 *
 * Exposed as a function instead of a class so tests can spin up a
 * full daemon, drive it, and tear it down without process-level signals.
 */
export async function runDaemon(config: ReceiverConfig): Promise<RunResult> {
  ...
  logger.info({ handler: handler.name, agentId: config.agentId, novaUrl: config.novaUrl }, 'daemon starting');
  ...
  const healthServer = new HealthServer({ config, pullLoop, dispatcher, startedAt });
  ...
  const stop = async (): Promise<void> => {
    ...
    logger.info(
      { stats: { pullLoop: pullLoop.getStats(), dispatcher: dispatcher.getStats() } },
      'shutdown complete',
    );
  };

  return { stop, pullLoop, dispatcher, healthServer, healthPort };
}
